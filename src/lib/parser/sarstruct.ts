...
const sarSchema = (cursor: any, registry: any) => {
  const authorId: number = struct.parseWithCursor(cursor, 'u32le', registry);
  const layerCount: number = struct.parseWithCursor(cursor, 'u8', registry);
  const sizeHeight: any = struct.parseWithCursor(cursor, 'u8', registry);
  const sizeWidth: any = struct.parseWithCursor(cursor, 'u8', registry);
  const soundEffect: any = struct.parseWithCursor(cursor, 'u8', registry);
  const layers: any = [];
